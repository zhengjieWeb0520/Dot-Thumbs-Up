...
export function reportMerchantInfo(content, img_urls, name, phone, to_user_id,callBack) {
	...
	return dispatch => {
		axios
			.post(serverIp + '/dianzanbao/report/accusation.do', data, {
				headers: {
...
//反馈
export function feedBackInfo(content, img_urls, name, phone, callBack) {
	let data = qs.stringify({
		content: content,
		img_urls: img_urls,
		name: name,
	...
	return dispatch => {
		axios
			.post(serverIp + '/dianzanbao/report/feedback.do', data, {
				headers: {
					token: window.sessionStorage.getItem('token'),
					user_id: window.sessionStorage.getItem('user_id')
				}
